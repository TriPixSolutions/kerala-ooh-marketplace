...
export function PoliformProjects() {
  ...
  return (
    <section className="py-16 sm:py-24 bg-[#F7F5F2] border-t border-[#E5E5E5]/70">
      <Container size="wide">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10 sm:mb-14 max-w-5xl mx-auto">
          <div>
          ...
          <div className="flex gap-4 sm:gap-6 overflow-x-auto pb-4 snap-x snap-mandatory no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0 sm:grid sm:grid-cols-2">
            {featured.map((proj) => (
              <Link
                ...
                className="group relative shrink-0 w-[82vw] sm:w-auto rounded-[28px] overflow-hidden bg-[#EAE7E1] aspect-[4/3] flex flex-col justify-end p-6 sm:p-8 snap-center shadow-sm border border-[#E5E5E5]/80 transition-all hover:shadow-lg hover:-translate-y-1"
              >
                <Image
                  src={proj.coverImage}
                  alt={proj.title}
                ...
                <div className="absolute top-4 left-4 z-10">
                  <span className="px-3 py-1 rounded-full bg-black/50 backdrop-blur-md text-[10px] font-mono uppercase tracking-widest text-white border border-white/10">
                    {proj.location}
                  </span>
                </div>

                {/* Bottom Content & Circular Button */}
                <div className="relative z-10 flex items-end justify-between gap-4 text-white">
                  <div>
                    <span className="text-[10px] font-mono tracking-wider uppercase text-[#D4AF37] block mb-1">
                      {proj.category}
                    </span>
                  ...
                  <div className="btn-circle-arrow group-hover:bg-[#8B6A4D]">
